...
export const metadata: Metadata = { title: "New Certificate of Compliance" };
...
export default async function NewCertificatePage({
  ...
  const [{ data: project }, { data: template }] = await Promise.all([
    supabase
      .from("projects")
      .select(
        "id, project_no, site_address, system_kwp, system_specs, customers (name, address, barangay), quotations (quote_no, quotation_items (description, qty, unit, sort_order))",
      )
      .eq("id", id)
      .single(),
    supabase
  ...
  if (!template) {
    return (
      <>
        <TopBar title="New Certificate" backHref={`/projects/${id}`} />
  ...
  return (
    <>
      <TopBar
        title={`Certificate — ${project.project_no}`}
        backHref={`/projects/${id}`}
      />
      <ContractEditor projectId={project.id} initialBody={body} docType="certificate" />
    </>
  );
}
